...
Usage:
  python scripts/plot_cem_comparison.py
  python scripts/plot_cem_comparison.py --size 8 --h 0.5
  python scripts/plot_cem_comparison.py --model 1d --size 16
"""
...
def compute_exact_energy(model, N, h):
    ...
    try:
        result = subprocess.run(
            [
                sys.executable,
                "scripts/exact_diag_ising_analytical.py",
                "-N",
                str(N),
                "-g",
                str(h),
...
def plot_cem_comparison(paired):
    ...
    for (model, N, h), triples in sorted(nh_groups.items()):
        ...
        out_dir = PLOTS_DIR / model / "cem_comparison"
...
def plot_cem_summary(paired):
    ...
    Saved as plots/cem_comparison/cem_summary.png.
    """
    cmap = plt.get_cmap("tab10")
    color_cache = {}
    ...
    out_dir = PLOTS_DIR / "cem_comparison"
    out_dir.mkdir(parents=True, exist_ok=True)
